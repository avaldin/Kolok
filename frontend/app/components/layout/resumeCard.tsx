...
const ResumeCard = ({ name, participants, tools }: Room) => {
  ...
  const availableTools = ALL_AVAILABLE_TOOLS.filter(
  ...
  return (
    <div className="w-5/6 max-w-2xl mx-auto my-6 p-6 rounded-2xl bg-brown-sugar border-4 border-bistre shadow-md flex flex-col space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between gap-6">
        <div className="flex-1 bg-atomic-tangerine rounded-xl p-4 shadow space-y-2">
          <h3 className="text-lg font-semibold text-bistre mb-2">
            Colocataires
          </h3>
          <ul className="space-y-2">
            {participants.map((user, index) => (
              <li
                key={index}
                className="flex items-center text-bistre space-x-3"
              >
          ...
          <div className="flex items-center justify-between mb-2 gap-4">
            ...
            {availableTools.length > 0 && (
              <div className="relative" ref={dropdownRef}>
                ...
                >
                  {showAddTool ? (
                    <X size={20} className="text-bistre" />
                  ) : (
                    <Plus size={20} className="text-bistre" />
                  )}
                </button>
                ...
                {showAddTool && (
                  <div className="absolute top-full right-0 mt-2 p-3 bg-brown-sugar rounded-lg border-2 border-bistre shadow-lg z-10 min-w-[200px]">
                    <h4 className="text-sm font-semibold text-bistre mb-2">
                      Outils disponibles :
                    </h4>
                    <ul className="space-y-2">
            ...
            {currentTools.map((tool, index) => (
              <li key={index}>
                <Link href={`/tools/${tool}`}>
                  <div className="cursor-pointer bg-peach-yellow hover:bg-peach-yellow/90 transition text-center py-2 px-2 rounded-md border border-bistre text-bistre font-medium">
                    {tool}
                  </div>
                </Link>
              </li>
